...
export default function Login() {
  ...
  const handleOAuth = async (provider: "google" | "apple") => {
    ...
    if (error) {
      showToast(error.message || "Error con OAuth", "error");
    }
  };
      ...
      <div className="scroll-area">
        <div className="max-w-md mx-auto px-6 pt-16 pb-10">
          ...
          <form className="space-y-4" onSubmit={doLogin}>
            <div className="animate-up d1">
              ...
              <div className="field flex items-center gap-3 px-4 h-[52px]">
                ...
                <input type="email" placeholder="tu@correo.com" value={email} onChange={e => setEmail(e.target.value)} className="flex-1 bg-transparent text-forest text-sm py-3.5 w-full"/>
              </div>
            </div>
            <div className="animate-up d2">
              ...
              <div className="field flex items-center gap-3 px-4 h-[52px]">
                ...
                <input type={showPwd ? "text" : "password"} placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} className="flex-1 bg-transparent text-forest text-sm py-3.5 w-full"/>
          ...
          <div className="flex gap-3 animate-up d5">
            <button onClick={() => handleOAuth("google")} className="flex-1 h-11 bg-white rounded-[16px] border border-[rgba(0,45,28,0.07)] flex items-center justify-center gap-2 text-sm font-semibold cursor-pointer hover:-translate-y-[2px] hover:shadow-[0_12px_32px_rgba(0,45,28,0.1)] transition-all">
            ...
            <button onClick={() => handleOAuth("apple")} className="flex-1 h-11 bg-white rounded-[16px] border border-[rgba(0,45,28,0.07)] flex items-center justify-center gap-2 text-sm font-semibold cursor-pointer hover:-translate-y-[2px] hover:shadow-[0_12px_32px_rgba(0,45,28,0.1)] transition-all">
