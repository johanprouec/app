...
export default function PublicarGanado() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const fileName = `listings/${Date.now()}_${file.name.replace(/[^a-zA-Z0-9.-]/g, '_')}`;
      
      // Try to use 'listings' bucket, fallback to 'documents' if it fails
      let bucket = 'listings';
        ...
        <div className="px-5 pt-4 pb-10 space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 animate-up d1">
            ...
            <Card className="p-5 space-y-4">
              ...
              <p className="text-xs text-stone">Sube una foto clara que represente el lote completo.</p>
              ...
              <div className="flex flex-col gap-4">
                ...
                {form.cover_image_url ? (
                  <div className="relative rounded-2xl overflow-hidden aspect-video shadow-md border border-stone/10 group">
                    <img
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                       <label htmlFor="imageFile" className="bg-white text-forest p-3 rounded-full cursor-pointer shadow-lg active:scale-90 transition-all">
                         <span className="material-symbols-outlined">edit</span>
                       </label>
                    </div>
                  </div>
                ) : (
                  <label htmlFor="imageFile" className="w-full aspect-video rounded-2xl border-2 border-dashed border-stone/20 bg-[#f5f0e8] flex flex-col items-center justify-center gap-3 cursor-pointer hover:border-forest/50 hover:bg-forest/5 transition-all animate-up">
