...
export default function Notifications() {
  ...
  const getIconStyle = (type: string) => {
    const styles: Record<string, { icon: string; bg: string; color: string; border: string }> = {
      ...
      chat: { icon: "chat", bg: "bg-sage-light", color: "text-forest", border: "" },
  ...
  const yesterdayNotifs = notifications.filter(n => isYesterday(new Date(n.created_at)));
  const olderNotifs = notifications.filter(n => !isToday(new Date(n.created_at)) && !isYesterday(new Date(n.created_at)));

  const NotificationItem = ({ n, i }: { n: Notification, i: number }) => {
    const style = getIconStyle(n.notification_type || n.type);
    
    return (
      <Card 
        key={n.id} 
        className={`p-4 flex gap-3 animate-up d${(i % 3) + 1} transition-all active:scale-[0.99] cursor-pointer 
          ${!n.is_read ? `border-l-[3px] shadow-md bg-white ${style.border || 'border-amber'}` : 'opacity-70 bg-white/60 border-none'}`}
        onClick={() => {
          markAsRead(n.id);
          if (n.link) router.push(n.link);
        }}
      >
        <div className={`w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0 ${style.bg} shadow-sm`}>
          <span className={`material-symbols-outlined fill-icon text-[20px] ${style.color}`}>{n.icon || style.icon}</span>
        </div>
        <div className="flex-1 min-w-0">
          <p className={`text-sm font-bold text-forest ${!n.is_read ? '' : 'font-medium'}`}>{n.title}</p>
          <p className="text-xs text-stone mt-0.5 leading-relaxed">{n.message || n.body}</p>
            ...
            <>
              {todayNotifs.length > 0 && (
                <div className="space-y-3">
                  ...
                  {todayNotifs.map((n, i) => <NotificationItem key={n.id} n={n} i={i} />)}
                </div>
              )}
              ...
              {yesterdayNotifs.length > 0 && (
                <div className="space-y-3">
                  ...
                  {yesterdayNotifs.map((n, i) => <NotificationItem key={n.id} n={n} i={i} />)}
                </div>
              )}
              ...
              {olderNotifs.length > 0 && (
                <div className="space-y-3">
                  ...
                  {olderNotifs.map((n, i) => <NotificationItem key={n.id} n={n} i={i} />)}
                </div>
              )}
            </>
          )}
